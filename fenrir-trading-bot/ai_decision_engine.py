...
from pumpfun_program import BondingCurveState
...
class TokenAnalysis:
    ...
    suggested_buy_amount_sol: Optional[float] = None
...
class TokenMetadata:
    ...
    # On-chain data
    bonding_curve_state: Optional[BondingCurveState] = None
...
class AITradingAnalyst:
    ...
    def _build_analysis_prompt(
        ...
        liquidity_ratio = (token.initial_liquidity_sol / 
                          token.current_market_cap_sol if token.current_market_cap_sol > 0 else 0)
...
"""
    ...
    def get_ai_performance_report(self) -> Dict:
        ...
        # Calculate metrics
        strong_buys = [p for p in evaluated 
                      if p["analysis"].decision == AIDecision.STRONG_BUY]
        buys = [p for p in evaluated 
               if p["analysis"].decision == AIDecision.BUY]
        skips = [p for p in evaluated 
                if p["analysis"].decision == AIDecision.SKIP]
        
        # Were the STRONG_BUY recommendations profitable?
        strong_buy_success = 0
        if strong_buys:
            profitable_strong_buys = [
                p for p in strong_buys 
                if p["actual_performance"]["pnl_pct"] > 0
            ]
        ...
        if buys:
            avg_buy_return = sum(
                p["actual_performance"]["pnl_pct"] for p in buys
            ) / len(buys)
